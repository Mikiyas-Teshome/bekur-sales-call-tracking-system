...
type Summary = { total: number; activeCount: number; totalLeads: number; totalCalls: number };

const statusClasses: Record<string, string> = { Active: "bg-success/12 text-success", Invited: "bg-warning/12 text-warning", Inactive: "bg-muted text-muted-foreground" };
...
export function TeamWorkspace({ result, allMembers, roles, summary, filters }: { result: Paginated<TeamMember>; allMembers: TeamMember[]; roles: RoleOption[]; summary: Summary; filters: Filters }) {
  ...
  const teamRoles = [...new Set(allMembers.map((member) => member.role))];

  return <div className="space-y-4 lg:space-y-5"><section className="flex flex-col gap-4 pt-2 sm:flex-row sm:items-end sm:justify-between lg:pt-0"><div><p className="text-sm font-medium text-primary">Administration</p><h2 className="mt-1 text-2xl font-bold tracking-tight">Team & permissions</h2><p className="mt-1 text-sm text-muted-foreground">Manage access, ownership, and sales performance across your workspace.</p></div><div className="flex gap-2"><Link href="/team/roles" className={cn(softPillClass, "h-11 px-5 text-sm")}><Shield className="size-4" strokeWidth={1.75} />Roles & permissions</Link><button type="button" onClick={() => setInviteOpen(true)} className={cn(primaryPillClass, "h-11 px-5 text-sm")}><UserRoundPlus className="size-4" strokeWidth={1.75} />Invite teammate</button></div></section><section className="grid gap-3 sm:grid-cols-3"><TeamStat icon={UsersRound} label="Team members" value={String(summary.total)} detail={`${summary.activeCount} active now`} /><TeamStat icon={ShieldCheck} label="Assigned leads" value={String(summary.totalLeads)} detail="across your workspace" /><TeamStat icon={Check} label="Calls logged" value={String(summary.totalCalls)} detail="all time" /></section><Surface><SurfaceHeader className="items-start"><div><SurfaceTitle>Sales team</SurfaceTitle><p className="mt-1 text-sm text-muted-foreground">{result.total} members match your current view.</p></div></SurfaceHeader><div className="mt-5 flex flex-col gap-3 lg:flex-row"><label className="relative min-w-0 flex-1"><Search className="absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground" strokeWidth={1.75} /><input aria-label="Search team" value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Search by name or email" className="h-11 w-full rounded-full border border-input bg-background pr-4 pl-10 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring/50" /></label><div className="flex gap-2"><FilterMenu label="Role" value={filters.role} options={["All roles", ...teamRoles]} onChange={(value) => updateParams({ role: value === "All roles" ? null : value })} /><FilterMenu label="Status" value={filters.status} options={["All statuses", "Active", "Invited", "Inactive"]} onChange={(value) => updateParams({ status: value === "All statuses" ? null : value })} /></div></div></Surface><Surface className="hidden overflow-hidden p-0 md:block"><div className="overflow-x-auto scrollbar-thin"><table className="w-full min-w-220 text-left"><thead className="border-b border-border bg-muted/60 text-[11px] text-muted-foreground"><tr><th className="px-5 py-3 font-medium">Member</th><th className="px-4 py-3 font-medium">Role</th><th className="px-4 py-3 font-medium">Status</th><th className="px-4 py-3 font-medium">Leads</th><th className="px-4 py-3 font-medium">Calls</th><th className="px-4 py-3 font-medium">Conversion</th><th className="px-4 py-3 font-medium">Revenue</th><th className="px-5 py-3 text-right font-medium">Action</th></tr></thead><tbody className="divide-y divide-border">{members.map((member) => <TeamRow key={member.id} member={member} onManage={setSelectedMember} />)}</tbody></table></div></Surface><div className="space-y-3 md:hidden">{members.map((member) => <TeamCard key={member.id} member={member} onManage={setSelectedMember} />)}</div><Surface className="py-3"><PaginationControls page={result.page} totalPages={result.totalPages} total={result.total} pageSize={result.pageSize} /></Surface><InviteDialog open={inviteOpen} onOpenChange={setInviteOpen} roles={roles} /><ManageDialog key={selectedMember?.id} member={selectedMember} roles={roles} teammates={allMembers} onClose={() => setSelectedMember(null)} /></div>;
}
