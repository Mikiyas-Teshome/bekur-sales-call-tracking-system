...
6. Reports (`src/features/reporting/reports-workspace.tsx`): period filter, Overview/Conversion/Team-performance tabs (Team performance hidden for roles that can't view it), a KPI strip, a hero revenue-trajectory chart, pipeline funnel, call-outcome mix, campaign efficiency table, target-progress card, team leaderboard, and an activity heatmap (leaderboard/heatmap replaced by a restricted placeholder for roles that can't view team data). Now query-driven by `src/services/reports.service.ts` (no fixture): a person filter scopes every card to one rep or manager, reps without `reports:view_team` are always scoped to themselves, and the leaderboard/target card read monthly KPI targets from `kpi_targets` (`src/services/kpi.service.ts`, set via `kpi:manage_targets` from the leaderboard or the Team page's Manage dialog). Manager rollups are workspace-wide — `users.managerId` is not used for scoping.
